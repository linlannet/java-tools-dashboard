...
import net.linlan.commons.cache.CacheManager;
import net.linlan.commons.cache.HeapCacheManager;
...
import net.linlan.datas.core.provider.result.ColumnIndex;
import net.linlan.datas.core.query.QueryBuilder;
import net.linlan.datas.core.utils.SqlHelper;
...
import static net.linlan.datas.core.query.AggregationBuilder.dateHistAggregation;
import static net.linlan.datas.core.query.AggregationBuilder.termsAggregation;
import static net.linlan.datas.core.query.SqlMethod.coalesce;


/**
 *
 * Filename:ElasticsearchDataProvider.java
 * Desc:
 *
 * @author Linlan
 ...
 *
 */
@DataProviderName(name = "Elasticsearch")
public class ElasticsearchDataProvider extends DataProvider implements Aggregatable, Initializing {

    private static final Logger logger = LoggerFactory.getLogger(ElasticsearchDataProvider.class);
    ...
    @DataQueryParameter(label = "Index *", type = DataQueryParameter.Type.Input, required = true, order = 2)
    protected String INDEX = "index";

    @DataQueryParameter(label = "Type *", type = DataQueryParameter.Type.Input, required = true, order = 3)
    ...
    @DataQueryParameter(label = "Override Aggregations", pageType = "dataset,widget", type = DataQueryParameter.Type.TextArea, order = 6)
    ...
    private JSONArray getFilter(AggConfig config) {
        ...
        filters.map(e -> SqlHelper.separateNull(e)).map(e -> configComponentToFilter(e)).filter(e -> e != null).forEach(result::add);
        return result;
    }
    ...
    private JSONObject configComponentToFilter(ConfigComponent cc) {
        if (cc instanceof DimensionConfig) {
            ...
            compositeConfig.getConfigComponents().stream().map(e -> SqlHelper.separateNull(e)).map(e -> configComponentToFilter(e)).forEach(boolArr::add);
    ...
    protected JSONObject post(String url, JSONObject request) throws Exception {
        ...
        String charset = dataSource.get(CHARSET) == null ? "utf-8" : dataSource.get(CHARSET);
        ...
        try {
            ...
            switch (types.get(d.getColumnName())) {
                ...
                default:
                    ...
                    aggregation = JsonBuilder.json(d.getColumnName(), termsAggregation(d.getColumnName(), 1000, missing));
            }
            // Query Override
            if (overrideAgg != null) {
    ...
    private JSONObject buildDateHistAggregation(String columnName, AggConfig config) throws Exception {
        ...
        return JsonBuilder.json(columnName, dateHistAggregation(columnName, intervalStr, 0, lower, upper));
    }
    ...
    private JSONObject queryBound(String columnName, AggConfig config) {
        ...
        return JsonBuilder.json(columnName, dateHistAggregation(columnName, intervalStr, 0));
    }
    ...
    private void getAggregationResponse(JSONObject object, List<String[]> result,
                                        List<String> parentKeys, int dimensionLevel,
                                        List<ColumnIndex> dimensionList,
                                        List<ColumnIndex> valueList) {
