...
import io.linlan.tools.data.aggregator.h2.H2Aggregator;
...
public class DataProviderManager implements ApplicationContextAware {
    ...
    static {
        Set<Class<?>> classSet = new Reflections("io.linlan.tools.data").getTypesAnnotatedWith(DataProviderName.class);
        for (Class c : classSet) {
            if (!c.isAssignableFrom(DataProvider.class)) {
                providers.put(((DataProviderName) c.getAnnotation(DataProviderName.class)).name(), c);
            } else {
                System.out.println("自定义DataProvider需要继承:io.linlan.tools.data.provider.DataProvider");
            }
        }
    }
