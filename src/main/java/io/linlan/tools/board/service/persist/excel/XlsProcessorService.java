...
import io.linlan.tools.board.dao.DashBoardDao;
...
/**
 * 
 * Filename:XlsProcessorService.java
 * Desc:
 *
 * @author hcday
 ...
 */
@Service
public class XlsProcessorService {

    private AbsXlsProcessor jpgAbsXlsProcesser = new JpgXlsProcessor();
    private AbsXlsProcessor tableAbsXlsProcesser = new TableXlsProcessor();

    public HSSFWorkbook dashboardToXls(List<PersistContext> contexts) {
        XlsProcessor context = null;
    ...
    public HSSFWorkbook tableToxls(JSONObject data) {
        XlsProcessor context = new XlsProcessor();
        ...
        new TableXlsProcessor().drawContent(context);
        setAutoWidth(sheet);
        return wb;
    }
        ...
        if (tables != widgets) {
            ...
            for (JSONArray rw : widgetRows) {
                ...
                for (int i = 0; i < rw.size(); i++) {
                    ...
                    context.setData(v);
                    AbsXlsProcessor processer = getProcesser(v.getString("type"));
    ...
    private AbsXlsProcessor getProcesser(String type) {
        switch (type) {
            case "jpg":
                return jpgAbsXlsProcesser;
            case "table":
                return tableAbsXlsProcesser;
        }
        return null;
    }
