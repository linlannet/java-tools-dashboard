...
CBCrossTable.prototype.table = function() {
    ...
    var pageination = pageinationTemplate.render({
        ...
        text_show: boardTranslate("CROSS_TABLE.SHOW"),
        text_items: boardTranslate("CROSS_TABLE.ENTRIES")
    });
    // --------------------------------------------- Render End ---------------------------------------------//
...
CBCrossTable.prototype.bandDrillEvent = function (render) {
    ...
    $.contextMenu({
        ...
        build: function ($trigger, e) {
            ...
            var items = {};
            if (up) {
                items.up = {name: boardTranslate("COMMON.ROLL_UP"), icon: "fa-arrow-up"}
            }
            if (down) {
                items.down = {name: boardTranslate("COMMON.DRILL_DOWN"), icon: "fa-arrow-down"}
            }
            return {
...
CBCrossTable.prototype.renderPagination = function (pagedData, pageNumber) {
    ...
    var liStr = '<li><a class="previewLink">' + boardTranslate("CROSS_TABLE.PREVIOUS_PAGE") + '</a></li>';
    if (pageCount < 10) {
    ...
    liStr += '<li><a class="nextLink">' + boardTranslate("CROSS_TABLE.NEXT_PAGE") + '</a></li>';
    if (target) {
...
CBCrossTable.prototype.export = function () {
    ...
    $(selector).on('click', function () {
        ...
        xhr.open('POST', 'dash/common/tableToxls.do');
