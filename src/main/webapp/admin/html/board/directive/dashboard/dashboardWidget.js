...
javaDashboard.directive('dashboardWidget', function ($compile, $templateCache, dataService, chartService) {

    var renderEchart = function (scope, element, attrs) {
        var template = $templateCache.get("echartContent");
        scope.myheight = scope.row.height ? (scope.row.height - 44) : 350;
        ...
        compile: function (element, attrs) {
            return {
                ...
                post: function (scope, element, attrs) {
                    switch (scope.widget.widget.content.config.chart_type) {
                        case 'map':
