...
'use strict';
javaDashboard.controller('widgetCtrl', function ($scope, $state, $stateParams, $http, $uibModal, dataService, ModalUtils, updateService, $filter, chartService, $timeout) {

        var translate = $filter('translate');
        var updateUrl = "dash/widget/update.do";
        var deleteUrl = "dash/widget/delete.do";
        ...
        $scope.chart_types = [
            ...
            {
                name: translate('CONFIG.WIDGET.LINE_K'), value: 'line', class: 'kLine',
                row: translate('CONFIG.WIDGET.TIPS_DIM_NUM_1_MORE'),
                column: translate('CONFIG.WIDGET.TIPS_DIM_NUM_0_MORE'),
                measure: translate('CONFIG.WIDGET.TIPS_DIM_NUM_1_MORE')
            },
            ...
            $http.post('dash/folder/familyTree.do', {folderIds: $scope.folderIds}).success(function (response) {
                ...
                var list = $scope.widgetList.map(function (w) {
                    var ds = _.find($scope.datasetList, function (obj) {
                        return obj.id == w.content.datasetId
                    });
                    var dsrName = '';
                    var dsr;
                    if (ds) {
                    ...
                    } else if (w.content.datasource) {
                        dsr = _.find($scope.datasourceList, function (obj) {
                            return obj.id == w.content.datasource
                        });
                    }
                    return {
                        "id": w.id,
        ...
        var getWidgetList = function (callback) {
            $http.get("dash/widget/list.do").success(function (response) {
        ...
        var getCategoryList = function () {
            $http.get("dash/widget/listCategory.do").success(function (response) {
    ...
    $scope.saveWin = function (type) {
        ...
        $scope.saveWgt = function (type) {
            ...
            o.content = {};
            o.content.config = $scope.curWidget.config;
            o.content.cfg = dataService.getCfg($scope.curWidget.config);

            if ($scope.customDs) {
                o.content.query = $scope.curWidget.query;
                o.content.datasource = $scope.datasource.id;
            } else {
                o.content.datasetId = $scope.curWidget.datasetId;
                if ($scope.dataset && $scope.dataset.id == $scope.curWidget.datasetId) {
                    o.content.datasource = $scope.dataset.datasourceId;
                }
            }
            o.content.expressions = _.filter($scope.curWidget.expressions, function (e) {
                return !$scope.isDsExpression(e);
            });
            o.content.filterGroups = _.filter($scope.curWidget.filterGroups, function (e) {
                return !$scope.isDsFilter(e);
            });
            $scope.alerts = [];
            $scope.verify = {widgetName: true};

            if (o.content.datasetId == undefined && $scope.customDs == false) {
                $scope.alerts = [{
                    msg: translate('CONFIG.WIDGET.DATASET') + translate('COMMON.NOT_EMPTY'),
                    type: 'danger'
                }];
            ...
            if (type == 'new' || $scope.widgetId == null) {
                $http.post("dash/widget/save.do", {json: angular.toJson(o)}).success(function (rcode) {
                    if (rcode.code == 0) {
                        getWidgetList();
                        getCategoryList();
                        $scope.getFolderList();
                ...
                o.id = $scope.widgetId;
                $http.post(updateUrl, {json: angular.toJson(o)}).success(function (rcode) {
                    if (rcode.code == 0) {
                        getWidgetList();
                        getCategoryList();
                        $scope.getFolderList();
        ...
        $scope.editWgt = function (widget) {
            $scope.showBox = true;
            $http.post("dash/widget/checkRule.do", {id: widget.id}).success(function (rcode) {
                if (rcode.code == 0) {
                    doEditWgt(widget);
                    if($scope.customDs == true) $scope.doConfigParams();
                } else {
                    var d = widget.content.datasetId ? 'CONFIG.WIDGET.DATASET' : 'CONFIG.WIDGET.DATA_SOURCE';
                    ModalUtils.alert(translate("ADMIN.CONTACT_ADMIN") + "：" + translate(d) + '/' + rcode.msg, "modal-danger", "lg");
                }
            });
        };
        ...
        var doEditWgt = function (widget) {
            ...
            switchNode(widget.id);
            $('#preview_widget').html('');
            $scope.curWidget = angular.copy(widget.content);
            ...
            $scope.datasource = _.find($scope.datasourceList, function (ds) {
                return ds.id == widget.content.datasource;
            });
            ...
            loadDataset(function () {
                ...
                loadDsFilterGroups();
                buildSchema();
                dataService.linkDataset($scope.curWidget.datasetId, $scope.curWidget.content);
            });
            addWatch();
        };
        ...
        var buildSchema = function () {
            ...
            if (!$scope.customDs) {
                ...
                if ($scope.dataset.content.schema && ($scope.dataset.content.schema.measure.length > 0 || $scope.dataset.content.schema.dimension.length > 0)) {
                    loadFromDataset = true;
                }
            }
            if (loadFromDataset) {
                $scope.schema = $scope.dataset.content.schema;
                $scope.alerts = [];
        ...
        $scope.deleteWgt = function (widget) {
            ModalUtils.confirm(translate("COMMON.CONFIRM_DELETE"), "modal-info", "lg", function () {
                $http.post("dash/widget/delete.do", {id: widget.id}).success(function (rcode) {
                    if (rcode.code == 0) {
                        getWidgetList();
                    } else {
        ...
        $scope.getQueryView = function () {
            if ($scope.datasource && $scope.datasource.name) {
                return 'dash/common/getConfigView.do?type=' + $scope.datasource.type;
            }
        };

        $scope.getChartView = function () {
            if ($scope.curWidget.config && $scope.curWidget.config.chart_type) {
                return 'admin/html/board/view/config/chart/' + $scope.curWidget.config.chart_type + '.html';
            }
        };

        $scope.getOptionsView = function() {
            var basePath = 'admin/html/board/view/config/chart/options/';
            if ($scope.curWidget.content && $scope.curWidget.config.chart_type) {
                ...
                resolve: {
                    ...
                    getSelects: function () {
                        return function (byFilter, column, callback) {
                            ...
                            if (byFilter) {
                                config = angular.copy($scope.curWidget.content);
                                var arr = _.findKey($scope.curWidget.content, function (o) {
                                    return o == setbackArr;
                                });
        ...
        $scope.editVFilter = function (o) {
            $uibModal.open({
                templateUrl: 'admin/html/board/view/config/modal/vfilter.html',
                windowTemplateUrl: 'admin/html/board/view/util/modal/window.html',
                backdrop: false,
                size: 'lg',
                controller: function ($scope, $uibModalInstance) {
        ...
        $scope.doConfigParams = function () {
            $http.get('dash/common/getConfigParams.do?type=' + $scope.datasource.type + '&page=widget.html').then(function (response) {
