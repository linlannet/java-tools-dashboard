...
'use strict';
javaDashboard.controller('boardCtrl',
    ...
    var originalData = [];
    var updateUrl = "dash/board/update.do";
    var deleteUrl = "dash/board/delete.do";

    var getPrivateFolder = function () {
        $http.post('dash/folder/getByParams.do', {parentId: 10000, name: ".private"}).success(function (response) {
    ...
    var getBoardList = function () {
        return $http.get("dash/board/list.do").success(function (response) {
    ...
    var loadBoardDataset = function (status) {
        ...
        _.each($scope.curBoard.layout.rows, function (row) {
            _.each(row.widgets, function (widget) {
                ...
                if (w.content.datasetId) {
                    datasetIdArr.push(w.content.datasetId);
                } else {
                    widgetArr.push(w);
                }
            });
        ...
        _.each(widgetArr, function (w) {
            status.i++;
            dataService.getColumns({
                datasource: w.content.datasource,
                query: w.content.query,
                datasetId: null,
                callback: function (dps) {
                    if (dps.msg == "1") {
    ...
    $scope.copyBoard = function (board) {
        ...
        o.name = o.name + '_copy';
        $http.post("dash/board/save.do", {json: angular.toJson(o)}).success(saveBoardCallBack);
    };

    $scope.deleteBoard = function (board) {
        ModalUtils.confirm(translate("COMMON.CONFIRM_DELETE"), "modal-warning", "lg", function () {
            $http.post(deleteUrl, {id: board.id}).success(function (rcode) {
                if (rcode.code == 0) {
                    getBoardList();
                    boardChange();
                    ModalUtils.alert(rcode.msg, "modal-success", "sm");
                } else {
    ...
    function saveBoardCallBack(rcode) {
        if (rcode.code == 0) {
            if (!$scope.curBoard.id) {
                $scope.curBoard.id = rcode.boardId;
            }
            if($scope.isPreview){
            ...
            ModalUtils.alert(rcode.msg, "modal-warning", "sm");
        }
    }
    ...
    $scope.saveBoard = function (notify, type) {
        ...
        if (type == 'new' || $scope.curBoard.id == null) {
            return $http.post("dash/board/save.do", {json: angular.toJson($scope.curBoard)}).success(callBack);
        } else if (type == 'edit') {
    ...
    $scope.editParam = function (row, index) {
        ...
        $uibModal.open({
            templateUrl: 'admin/html/board/view/config/board/modal/param.html',
            windowTemplateUrl: 'admin/html/board/view/util/modal/window.html',
            backdrop: false,
            size: 'lg',
            controller: function ($scope, $uibModalInstance) {
    ...
    $scope.changeSourceCol = function (e, widgetId) {
        ...
        $http.get("dash/widget/info.do?id=" + e.widgetId).then(function (response) {
            if (!response) {
                return false;
            }
