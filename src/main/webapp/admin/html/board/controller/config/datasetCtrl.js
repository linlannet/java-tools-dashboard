...
javaDashboard.controller('datasetCtrl', function ($scope, $http, $state, $stateParams, dataService, $uibModal, ModalUtils, $filter, chartService, $timeout, uuid4) {

    var translate = $filter('translate');
    $scope.curDataset = {content: {expressions: [], filters: [], schema: {dimension: [], measure: []}}};
    $scope.curWidget = {};
    $scope.alerts = [];
    ...
    var originalData = [];
    var updateUrl = "dash/dataset/update.do";
    var deleteUrl = "dash/dataset/delete.do";
        ...
        $http.post('dash/folder/familyTree.do', {folderIds: $scope.folderIds}).success(function (response) {
            ...
            var list = $scope.datasetList.map(function (ds) {
                var dsr = _.find($scope.datasourceList, function (obj) {
                    return obj.id == ds.content.datasource
                });
                return {
                    "id": ds.id,
                    "name": ds.name,
    ...
    var getCategoryList = function () {
        $http.get("dash/dataset/listCategory.do").success(function (response) {
    ...
    $scope.editDs = function (ds) {
        $scope.showBox = true;
        $http.post("dash/datasource/checkRule.do", {id: ds.content.datasource}).success(function (rcode) {
            if (rcode.code == 0) {
                doEditDs(ds);
                $scope.doConfigParams();
            } else {
                ModalUtils.alert(translate("ADMIN.CONTACT_ADMIN") + "：Datasource/" + rcode.msg, "modal-danger", "lg");
            }
        });
    };
    ...
    var doEditDs = function (ds) {
        ...
        if (!$scope.curDataset.content.expressions) {
            $scope.curDataset.content.expressions = [];
        }
        if (!$scope.curDataset.content.filters) {
            $scope.curDataset.content.filters = [];
        }
        if (!$scope.curDataset.content.schema) {
            $scope.curDataset.content.schema = {dimension: [], measure: []};
        }
        $scope.datasource = _.find($scope.datasourceList, function (ds) {
            return ds.id == $scope.curDataset.content.datasource;
        });
        $scope.curWidget.query = $scope.curDataset.content.query;
        $scope.selects = ds.content.selects;
        //$scope.loadData();
    };

    $scope.checkExist = function (column) {
        var find = _.find($scope.curDataset.content.schema.measure, function (e) {
            return e.column == column;
        });
        if (!_.isUndefined(find)) {
            return true;
        }
        find = _.find($scope.curDataset.content.schema.dimension, function (e) {
            if (e.type == 'level') {
    ...
    $scope.deleteDs = function (ds) {
        $http.get("dash/widget/listAll.do").then(function (response) {
            if (!response) {
                return false;
            }
            var resDs = [];
            for (var i = 0; i < response.data.length; i++) {
                if (response.data[i].datasetId == ds.id) {
            ...
            ModalUtils.confirm(translate("COMMON.CONFIRM_DELETE"), "modal-warning", "lg", function () {
                $http.post(deleteUrl, {id: ds.id}).success(function (rcode) {
                    if (rcode.code == 0) {
                        getDatasetList();
                    } else {
    ...
    $scope.copyDs = function (ds) {
        ...
        $http.post("dash/dataset/save.do", {json: angular.toJson(data)}).success(function (rcode) {
            if (rcode.code == 0) {
                getDatasetList();
    ...
    $scope.saveWin = function (type) {
        ...
        $uibModal.open({
            templateUrl: 'admin/html/board/view/config/modal/saveWin.html',
    ...
    $scope.save = function (type) {
        $scope.datasource ? $scope.curDataset.content.datasource = $scope.datasource.id : null;
        $scope.curDataset.content.query = $scope.curWidget.query;
        ...
        if ($scope.datasource) {
            ds.datasourceId = $scope.datasource.id;
        }
        ds.name = $scope.curDataset.name;
        ds.folderId = $scope.selectedFold.id;

        if(type == 'new' || ds.id == null){
            $http.post("dash/dataset/save.do", {json: angular.toJson(ds)}).success(function (rcode) {
                if (rcode.code == 0) {
                    getCategoryList();
                    getDatasetList();
                    $scope.verify = {dsName: true};
            ...
            $http.post(updateUrl, {json: angular.toJson(ds)}).success(function (rcode) {
                if (rcode.code == 0) {
                    getCategoryList();
                    getDatasetList();
                    $scope.verify = {dsName: true};
    ...
    $scope.editFilterGroup = function (col) {
        var columnObjs = schemaToSelect($scope.curDataset.content.schema);
        $uibModal.open({
            templateUrl: 'admin/html/board/view/config/modal/filterGroup.html',
            windowTemplateUrl: 'admin/html/board/view/util/modal/window.html',
            backdrop: false,
            scope: $scope,
            controller: function ($scope, $uibModalInstance) {
                ...
                $scope.addColumn = function (str) {
                    $scope.content.filters.push({col: str, type: '=', values: []})
                };
                $scope.ok = function () {
                    if (col) {
                        col.group = $scope.content.group;
                        col.filters = $scope.content.filters;
                    } else {
                        if ($scope.$parent.curDataset.content.filters == null) {
                            $scope.$parent.curDataset.content.filters = [];
                        }
                        $scope.$parent.curDataset.content.filters.push($scope.data);
                    }
                    $uibModalInstance.close();
                };
                $scope.editFilter = function (filter) {
                    $uibModal.open({
                        templateUrl: 'admin/html/board/view/dashboard/modal/param.html',
                        windowTemplateUrl: 'admin/html/board/view/util/modal/window.html',
                        backdrop: false,
                        size: 'lg',
                        resolve: {
    ...
    $scope.deleteFilterGroup = function (index) {
        ModalUtils.confirm(translate("COMMON.FILTER_GROUP") + ": [" + $scope.curDataset.content.filters[index].group + "], " +
            translate("COMMON.CONFIRM_DELETE"), "modal-warning", "lg",
            function () {
                $scope.curDataset.content.filters.splice(index, 1)
            }
        );
    };
    ...
    $scope.editExp = function (col) {
        ...
        if (!col) {
            ok = function (exp, alias) {
                $scope.curDataset.content.expressions.push({
                    type: 'exp',
                    exp: data.expression,
                    alias: data.alias,
                    id: uuid4.generate()
        ...
        var columnObjs = schemaToSelect($scope.curDataset.content.schema, $scope.selects);
        var expressions = $scope.curDataset.content.expressions;
        $uibModal.open({
            templateUrl: 'admin/html/board/view/config/modal/exp.html',
            windowTemplateUrl: 'admin/html/board/view/util/modal/window.html',
            backdrop: false,
            size: 'lg',
            scope: $scope,
            controller: function ($scope, $uibModalInstance) {
                ...
                $scope.verify = function () {
                    $scope.alerts = [];
                    var v = verifyAggExpRegx($scope.content.expression);
                    $scope.alerts = [{
                ...
                $scope.ok = function () {
                    ...
                    $scope.content.expression = $scope.expAceSession.getValue();
                    ok($scope.data);
                    $uibModalInstance.close();
                };
            }
    ...
    $scope.deleteExp = function (index) {
        ModalUtils.confirm(translate("CONFIG.COMMON.CUSTOM_EXPRESSION") + ": [" + $scope.curDataset.content.expressions[index].alias + "], " +
            translate("COMMON.CONFIRM_DELETE"), "modal-warning", "lg",
            function () {
                $scope.curDataset.content.expressions.splice(index, 1)
            }
        );
    };
    ...
    $scope.measureToDimension = function (index, o) {
        $scope.curDataset.content.schema.measure.splice(index, 1);
        $scope.curDataset.content.schema.dimension.push(o);
    };

    $scope.toDimension = function (o) {
        $scope.curDataset.content.schema.dimension.push($scope.createNode(o));
    };
    ...
    $scope.custom = function (o) {
        ...
        $uibModal.open({
            templateUrl: 'admin/html/board/view/config/modal/custom.html',
            windowTemplateUrl: 'admin/html/board/view/util/modal/window.html',
            backdrop: false,
            size: 'lg',
            controller: function ($scope, $uibModalInstance) {
    ...
    $scope.loadData = function (reload) {
        ...
        dataService.getColumns({
            ...
            callback: function (dps) {
                ...
                $scope.curDataset.content.selects = $scope.selects;
            }
        });
    };
    ...
    $scope.doConfigParams = function () {
        $http.get('dash/common/getConfigParams.do?type=' + $scope.datasource.type + '&page=dataset.html').then(function (response) {
