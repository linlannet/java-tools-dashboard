...
javaDashboard.controller('userAdminCtrl', function ($scope, $http, ModalUtils, $filter) {
    ...
    $http.get("dash/adminuser/isAdmin.do").success(function (response) {
        $scope.isAdmin = response;
    });

    var getUserList = function () {
        $http.get("dash/adminuser/getUserList.do").success(function (response) {
    ...
    var getRoleList = function () {
        $http.get("dash/adminrole/getRoleList.do").success(function (response) {
    ...
    var getUserRoleList = function () {
        $http.get("dash/adminuser/getUserRoleList.do").success(function (response) {
    ...
    var getBoardList = function () {
        return $http.get("dash/board/listAdmin.do").success(function (response) {
    ...
    var getFolderList = function () {
        return $http.get('dash/folder/list.do').success(function (response) {
    ...
    var getMenuList = function () {
        return $http.get("dash/menu/getMenuList.do").success(function (response) {
    ...
    var getDatasourceList = function () {
        return $http.get("dash/datasource/getDatasourceList.do").success(function (response) {
    ...
    var getDatasetList = function () {
        return $http.get("dash/dataset/listAdmin.do").success(function (response) {
    ...
    var getWidgetList = function () {
        return $http.get("dash/widget/listAdmin.do").success(function (response) {
    ...
    var getJobList = function () {
        return $http.get("dash/operationjob/listAdmin.do").success(function (response) {
    ...
    var getRoleResList = function () {
        $http.get("dash/adminrole/getRoleResList.do").success(function (response) {
    ...
    $scope.searchUserByName = function (user) {
        ...
        if (!$scope.filterByRole) {
            return (user.username + user.name).toLowerCase().indexOf($scope.userKeyword) != -1;
        } else {
            return false;
        }
    };
    ...
    $scope.saveUser = function () {
        ...
        if ($scope.optFlag == 'newUser') {
            $http.post("dash/adminuser/saveNewUser.do", {user: angular.toJson($scope.curUser)}).success(function (rcode) {
                if (rcode.code == 0) {
                    $scope.optFlag = 'none';
                    getUserList();
            ...
            $http.post("dash/adminuser/updateUser.do", {user: angular.toJson($scope.curUser)}).success(function (rcode) {
                if (rcode.code == 0) {
                    $scope.optFlag = 'none';
                    getUserList();
                    $scope.verify = {dsName: true};
    ...
    $scope.saveRole = function () {
        ...
        if ($scope.optFlag == 'newRole') {
            $http.post("dash/adminrole/saveRole.do", {role: angular.toJson($scope.curRole)}).success(function (rcode) {
                if (rcode.code == 0) {
                    $scope.optFlag = 'none';
                    getRoleList();
                    $scope.verify = {dsName: true};
            ...
            $http.post("dash/adminrole/updateRole.do", {role: angular.toJson($scope.curRole)}).success(function (rcode) {
                if (rcode.code == 0) {
                    $scope.optFlag = 'none';
                    getRoleList();
                    $scope.verify = {dsName: true};
    ...
    $scope.grantRole = function () {
        ...
        $http.post("dash/adminuser/updateUserRole.do", {
    ...
    $scope.revokeRole = function () {
        ...
        $http.post("dash/adminuser/deleteUserRole.do", {
            userIdArr: angular.toJson(userIds),
            roleIdArr: angular.toJson(roleIds)
        }).success(function (rcode) {
            if (rcode.code == 0) {
    ...
    var changeResSelectByTree = function (tree) {
        ...
        if ($scope.selectRole) {
            ...
            _.each(tree.resList, function (e) {
                ...
                if (e.text) {
                    tree.treeInstance.jstree(true).rename_node(e, e.text + getCUDRlabel(_n.original.edit, _n.original.delete));
                }
            });
        }
    };
    ...
    $scope.grantRes = function () {
        ...
        $http.post("dash/adminrole/updateRoleRes.do", {
            roleIdArr: angular.toJson(roleIds),
    ...
    $scope.deleteRole = function () {
        ModalUtils.confirm(translate("COMMON.CONFIRM_DELETE"), "modal-info", "lg", function () {
            $http.post("dash/adminuser/deleteRole.do", {
                roleId: $scope.selectRole[0].id
            }).success(function (rcode) {
                if (rcode.code == 0) {
                    $scope.selectRole = null;
                    $scope.selectRes = null;
    ...
    $scope.deleteUser = function () {
        ModalUtils.confirm(translate("COMMON.CONFIRM_DELETE"), "modal-info", "lg", function () {
            $http.post("dash/adminuser/deleteUser.do", {
                userId: $scope.selectUser[0].id
            }).success(function (rcode) {
                if (rcode.code == 0) {
                    $scope.selectUser = null;
                    getUserList();
