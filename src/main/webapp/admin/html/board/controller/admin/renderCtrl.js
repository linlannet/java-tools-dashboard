javaDashboard.controller('renderCtrl', function ($timeout, $rootScope, $scope, $state, $location, $http, ModalUtils, chartService) {
    ...
    var buildRender = function (w, reload) {
        w.render = function (content, optionFilter, scope) {
            w.persist = {};
            var chartType = w.widget.content.config.chart_type;
            try {
    ...
    var runTask = function () {
        ...
        html2canvas($('body')[0], {
            onrendered: function (canvas) {
                ...
                xmlhttp.open("POST", "dash/common/persist.do", false);
    ...
    $scope.load = function (reload) {
        ...
        $http.get("dash/board/info.do?id=" + $location.search().id).success(function (response) {
            $scope.loading = false;
