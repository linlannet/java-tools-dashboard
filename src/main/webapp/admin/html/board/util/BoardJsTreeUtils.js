...
function jstree_baseTreeEventsObj(option) {
        return  {
            ...
            create_node: function (obj, node ) {
                ...
                option.ngHttp.post("dash/folder/save.do", {json: angular.toJson(item)}).success(function (serviceStatus) {
                    if (serviceStatus.code == '0') {
                    // if (serviceStatus.status == '1') {
                        item.id = serviceStatus.dseFolder.id;
                        rename(item);
                    } else {
            ...
            rename_node: function (obj, node) {
                ...
                option.ngHttp.post("dash/folder/update.do", {json: angular.toJson(item)}).success(function (serviceStatus) {
                    if (serviceStatus.code == '0') {
                    // if (serviceStatus.status == '1') {
            ...
            delete_node: function (obj, node) {
                ...
                for(var i=0;i<items.length;i++) {
                    option.ngHttp.post("dash/folder/delete.do?id=" + items[i]).success(function (serviceStatus) {
                        if (serviceStatus.code == '0') {
                        // if (serviceStatus.status == '1') {
                            //console.log('success!');
                        } else {
