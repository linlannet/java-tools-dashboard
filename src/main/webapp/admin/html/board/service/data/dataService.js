...
'use strict';
javaDashboard.service('dataService', function ($http, $q, updateService) {
    ...
    var getDatasetList = function () {
        ...
        if (datasetList) {
            ...
            $http.get("dash/dataset/list.do").success(function (data) {
        ...
        if (_.isUndefined(datasetId) || _.isUndefined(chartConfig)) {
            ...
            return getDatasetList().then(function (dsList) {
                ...
                _.each(chartConfig.filters, function (f) {
                    if (f.group) {
                        var group = _.find(dataset.content.filters, function (e) {
                            return e.id == f.id;
                        });
                        if (group) {
                ...
                _.each(chartConfig.values, function (v) {
                    _.each(v.cols, function (c) {
                        if (c.type == 'exp') {
                            var exp = _.find(dataset.content.expressions, function (e) {
                                return c.id == e.id;
                            });
                            if (exp) {
                                c.exp = exp.exp;
                            ...
                            c.exp = replaceVariable(dataset.content.expressions, c);
                        }
                    });
                });
                ...
                var linkFunction = function (k) {
                    if (k.id) {
                        var _level;
                        var _dimension;
                        _.each(dataset.content.schema.dimension, function (e) {
                            if (e.type == 'level') {
    ...
    this.getCfg = function (chartConfig) {
        return getCfg(chartConfig);
    };
    var getCfg = function (chartConfig) {
    ...
    this.getDimensionValues = function (datasource, query, datasetId, colmunName, chartConfig, callback) {
        ...
        linkDataset(datasetId, chartConfig).then(function () {
            var cfg = undefined;
            if (chartConfig) {
                cfg = getCfg(chartConfig);
            }

            $http.post("dash/common/getDimensionValues.do", {
    ...
    this.getDataSeries = function (args) {
        ...
        linkDataset(datasetId, chartConfig).then(function () {
            var cfg = getCfg(chartConfig);
            $http.post("dash/common/getAggregateData.do", {
                datasourceId: datasource,
                query: angular.toJson(query),
                datasetId: datasetId,
                cfg: angular.toJson(cfg),
    ...
    this.getDrillPath = function (datasetId, id) {
        ...
        getDatasetList().then(function (dsList) {
            ...
            var path = [];
            var level;
            _.each(dataset.content.schema.dimension, function (_e) {
                if (_e.type == 'level') {
    ...
    var getDrillConfig = function (datasetId, chartConfig) {
        ...
        getDatasetList().then(function (dsList) {
                ...
                if (!dataset.content.schema || dataset.content.schema.dimension.length == 0) {
                ...
                var _f = function (array) {
                    _.each(array, function (c, i_array) {
                        var level;
                        var i_level;
                        _.find(dataset.content.schema.dimension, function (_e) {
                            if (_e.type == 'level') {
    ...
    this.viewQuery = function (params, callback) {
        ...
        linkDataset(params.datasetId, params.config).then(function () {
            var cfg = getCfg(params.config);

            $http.post("dash/common/viewAggDataQuery.do", {
    ...
    this.getColumns = function (option) {
        $http.post("dash/common/getColumns.do", {
